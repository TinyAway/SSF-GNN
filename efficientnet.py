...
class EfficientNet(nn.Module):
    def __init__(self,
        ...
        #   往里面加东西，是update
        # layers = OrderedDict()
        self.stem = ConvBNActivation(in_planes=12,
                                     out_planes=adjust_channels(32),
                                     kernel_size=3,
                                     stride=2,
                                     norm_layer=norm_layer)
        # first conv，命名为stem_conv
        # layers.update({"stem_conv": ConvBNActivation(in_planes=12,
        ...
        # 起名叫top，也是人才
        self.head = ConvBNActivation(in_planes=last_conv_input_c,
    ...
    def get_stages(self):
        return [
            self.layers[: self.stage_number[0]],
            self.layers[self.stage_number[0]: self.stage_number[1]],
            self.layers[self.stage_number[1]: self.stage_number[2]],
            self.layers[self.stage_number[2]:],
        ]

    def extract_features(self, x, encoder_depth):
        x = self.stem(x)
...
if __name__ == "__main__":
    model = efficientnet_b4(8)
    # x = torch.rand(4, 12, 256, 256)
    # feats = model.extract_features(x)
    # for feat in feats:
    #     print(feat.shape)

    summary(model, torch.zeros(1, 12, 256, 256))
