...
default_cfgs = {
    'gnn_patch16_224': _cfg(
        crop_pct=0.9, input_size=(3, 256, 256),
...
#
#         self.stem = Stem(in_dim=12, out_dim=channels, act=act)
#
...
#
#         self.pos_embed = nn.Parameter(torch.zeros(1, channels, 16, 16))
#
#         if opt.use_dilation:
...
#         return self.prediction(x).squeeze(-1).squeeze(-1), x


def vig_ti_256_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=8, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
...
class Stem(nn.Module):
    ...
    def __init__(self, img_size=256, in_dim=12, out_dim=768, act='relu'):
        super().__init__()
        self.convs = nn.Sequential(
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.stage_number = opt.stage_number
    ...
    def forward(self, inputs):
        ...
        x = F.adaptive_avg_pool2d(x, 1)
        return self.prediction(x).squeeze(-1).squeeze(-1), feats

    def get_stages(self):
        return [
            self.backbone[: self.stage_number[0]],
            self.backbone[self.stage_number[0]: self.stage_number[1]],
            self.backbone[self.stage_number[1]: self.stage_number[2]],
            self.backbone[self.stage_number[2]:],
        ]



def pvig_ti_256_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=8, drop_path_rate=0.0, **kwargs):
...
if __name__ == "__main__":
    model = pvig_ti_256_gelu()
    # summary(model, torch.zeros(2, 12, 256, 256))
    img = torch.rand((4, 12, 256, 256))
    out, feat = model(img)
    print(out.shape)
